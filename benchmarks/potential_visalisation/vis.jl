...
p1 = heatmap(x_range, y_range, μ1,
             title="Potential 1: x²y + eˣsin(y)",
             c=:magma,
             colorbar=false,
             legend=false)

p2 = heatmap(x_range, y_range, μ2,
             title="Potential 2: xy² + ln(x²+1) + y³",
             c=:magma,
             colorbar=false,
             legend=false)

p3 = heatmap(x_range, y_range, μ3,
             title="Potential 3",
             c=:magma,
             colorbar=false,
             legend=false)
...
plot(p1, p2, p3, layout=(1, 3), size=(1500, 500),
     ...
     colorbar=:right,
     left_margin=5*Plots.mm, right_margin=10*Plots.mm, bottom_margin=5*Plots.mm)
...
p_u0 = heatmap(x_range, y_range, u0_data,
