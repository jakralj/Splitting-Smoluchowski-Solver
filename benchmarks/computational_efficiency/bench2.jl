...
function bench(steps_vec=[100, 250, 500, 1000], nxs=[100], potential = :potential_2, dt = 0.01)
    ...
    for i in 1:length(collected_ks)
    ...
    df_results = DataFrame(
        ...
        LIE = ["$(round(m, digits=4)) \\pm $(round(s, digits=4))" for (m, s) in zip(lie_medians, lie_stds)],
        STRANG = ["$(round(m, digits=4)) \\pm $(round(s, digits=4))" for (m, s) in zip(strang_medians, strang_stds)],
        ADI = ["$(round(m, digits=4)) \\pm $(round(s, digits=4))" for (m, s) in zip(adi_medians, adi_stds)]
    )
...
function writetable_latex(df::DataFrame, filename::String)
    open(filename, "w") do io
        ...
        for row in eachrow(df)
            println(io, "$(row.Steps) & $(row.Nx) & $(row.LIE) & $(row.STRANG) & $(row.ADI) \\\\")
        end
