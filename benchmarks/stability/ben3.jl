...
function create_plots(results)
    ...
    for pot_name in potential_names
        p = plot(title="$pot_name - Time Step Comparison", 
                xlabel="Time", ylabel="Free Energy",
                legend=:bottomright, linewidth=1.5)
    ...
    for pot_name in potential_names
        p = plot(title="$pot_name - Final Free Energy vs Time Step", 
                xlabel="Time Step (dt)", ylabel="Final Free Energy",
                legend=:topright, linewidth=2, xscale=:log10)
        ...
        for (method_idx, method_name) in enumerate(method_names)
            ...
            plot!(p, dt_values, final_values,
                  label=method_name, color=plot_colors[method_idx],
                  marker=shapes[method_idx], markersize=4, markeralpha=0.5)
        end
