...
function strang_splitting_states!(
    u,
    dt::Float64,
    dx,
    num_steps,
...
function create_evolution_plot()
    ...
    save_steps = [
        Int(round(0.1 * num_steps)),
        Int(round(0.5 * num_steps)),
        Int(round(0.9 * num_steps)),
    ]
    time_labels = [
        "Early (t=$(0.1*final_time))",
        "Middle (t=$(0.5*final_time))",
        "Late (t=$(0.9*final_time))",
    ]
    ...
    lie_states = lie_splitting_states!(
        u_lie,
        dt,
        dx,
        num_steps,
    ...
    strang_states = strang_splitting_states!(
        u_strang,
        dt,
        dx,
        num_steps,
    ...
    for (method_idx, (method_name, method_states)) in
        enumerate(zip(method_names, all_method_states))
