...
function run_mms_verification_2()
    ...
    for dt in dt_values
        ...
        println("dt = $dt: Lie error = $error_lie, Strang error = $error_strang, ADI error = $error_adi")
    end
    
    # Plot convergence
    p1 = plot(dt_values, [errors_lie errors_strang errors_adi], 
              xlabel="Time step", ylabel="L2 Error", 
              label=["Lie" "Strang" "ADI"], 
              xscale=:log10, yscale=:log10,
    ...
    return p1, errors_lie, errors_strang, errors_adi
end
