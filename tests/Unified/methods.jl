...
function TempArrays2D(::Type{T}, nx::Int, ny::Int) where T
    temp_1d_x = [TempArrays1D(T, ny) for _ in 1:nx]
    temp_1d_y = [TempArrays1D(T, nx) for _ in 1:ny]
...
function lie_splitting(dt::Float64, dx, u0, num_steps, μ, ∇μX, ∇μY, ∇2μX, ∇2μY, D, kbT)
    ...
    return lie_splitting!(u, dt, dx, num_steps, μ, ∇μX, ∇μY, ∇2μX, ∇2μY, D, kbT, temp_arrays)
end
...
function strang_splitting(dt::Float64, dx, u0, num_steps, μ, ∇μX, ∇μY, ∇2μX, ∇2μY, D, kbT)
    ...
    temp_arrays = TempArrays2D(eltype(u0), nx, ny)
    u = copy(u0)  # Only copy needed for API compatibility
    return strang_splitting!(u, dt, dx, num_steps, μ, ∇μX, ∇μY, ∇2μX, ∇2μY, D, kbT, temp_arrays)
end

function adi_scheme(dt::Float64, dx::Float64, u0, num_steps, μ, ∇μX, ∇μY, ∇2μX, ∇2μY, D, kbT)
    nx, ny = size(u0)
