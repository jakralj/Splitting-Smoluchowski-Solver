...
function random_placement(number, hexagon_points, width, big_radius, center_x, center_y)
    ...
    for point in placed_circles
        dist = distance(Float64(center_x), Float64(center_y), Float64(point[1]), Float64(point[2]))
        if dist >= big_radius
...
function hexagonal_place(center_x, center_y, radius, big_radius, ratio)
    ...
    function hexagon(x, y, width)
        points = [(x + width * cos(deg2rad(Float64(alpha))), y + width * sin(deg2rad(Float64(alpha)))) 
    ...
    for row in -round(Int, big_radius/row_height):round(Int, big_radius/row_height)
        for col in -round(Int, big_radius/col_width):round(Int, big_radius/col_width)
