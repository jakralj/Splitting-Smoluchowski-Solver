...
"""
ADI scheme with direct source term integration - X direction sweep
"""
function x_direction_sweep_with_source(u, αx, αy, βx, βy, dt, ∇μX, ∇μY, ∇2μX, ∇2μY, source_term, t)
    u_half = similar(u)
    current_source = source_term(t + dt/2)  # Evaluate source at mid-time for better accuracy
    ...
    for i in 1:nx
        ...
        for j in 1:nx
            if j == 1
                ...
                B[j] = u[i,j] + y_term + dt/2 * current_source[i,j]  # Add source term
...
"""
ADI scheme with direct source term integration - Y direction sweep
"""
function y_direction_sweep_with_source(u_half, αx, αy, βx, βy, dt, ∇μX, ∇μY, ∇2μX, ∇2μY, source_term, t)
    u = similar(u_half)
    current_source = source_term(t + dt/2)  # Evaluate source at mid-time
    ...
    for j in 1:nx
        ...
        for i in 1:nx
            if i == 1
                ...
                B[i] = u_half[i,j] + x_term + dt/2 * current_source[i,j]  # Add source term
...
"""
Direct ADI scheme with integrated source terms
"""
function adi_scheme_direct_source(dt::Float64, u0, num_steps, μ, ∇μX, ∇μY, ∇2μX, ∇2μY, source_term)
    ...
    for step in 1:num_steps
        ...
        u = x_direction_sweep_with_source(u, αx, αy, βx, βy, dt, ∇μX, ∇μY, ∇2μX, ∇2μY, source_term, current_time)
        
        # Second sweep: y-direction (implicit in y, explicit in x) with source
        u = y_direction_sweep_with_source(u, αx, αy, βx, βy, dt, ∇μX, ∇μY, ∇2μX, ∇2μY, source_term, current_time)
    end
...
"""
Original ADI scheme without source terms (for comparison)
"""
function adi_scheme(dt::Float64, u0, num_steps)
    ...
    # Dummy source term (returns zeros)
    dummy_source = t -> zeros(size(u0))
    ...
    for step in 1:num_steps
        ...
        u = x_direction_sweep_with_source(u, αx, αy, βx, βy, dt, ∇μX, ∇μY, ∇2μX, ∇2μY, dummy_source, current_time)
        
        # Second sweep: y-direction (implicit in y, explicit in x) 
        u = y_direction_sweep_with_source(u, αx, αy, βx, βy, dt, ∇μX, ∇μY, ∇2μX, ∇2μY, dummy_source, current_time)
    end
...
"""
Original operator splitting approach with source terms (for comparison)
"""
function adi_scheme_with_source(dt::Float64, u0, num_steps, μ, ∇μX, ∇μY, ∇2μX, ∇2μY, source)
    ...
    # Dummy source for the original x/y sweep functions
    dummy_source = t -> zeros(size(u0))
    
    # Main time-stepping loop
    for step in 1:num_steps
        # First sweep: x-direction (implicit in x, explicit in y)
        u = x_direction_sweep_with_source(u, αx, αy, βx, βy, dt/2, ∇μX, ∇μY, ∇2μX, ∇2μY, dummy_source, 0.0)
        
        # Second sweep: y-direction (implicit in y, explicit in x) 
        u = y_direction_sweep_with_source(u, αx, αy, βx, βy, dt/2, ∇μX, ∇μY, ∇2μX, ∇2μY, dummy_source, 0.0)
...
"""
Run MMS verification for potential 1 - comparing direct integration vs splitting
"""
function run_mms_verification_1_direct()
    println("=== MMS Verification for Potential 1 - Direct ADI vs Splitting ===")
    ...
    for dt in dt_values
        ...
        # Test ADI scheme with splitting
        source_adi_split = create_mms_source(source_func, 0.0)
        u_adi_split = adi_scheme_with_source(dt, u0, num_steps, μ, ∇μX, ∇μY, ∇2μX, ∇2μY, source_adi_split)
        error_adi_split = norm(u_adi_split - u_exact, 2) * dx
        push!(errors_adi_split, error_adi_split)
        
        # Test ADI scheme with direct integration
        u_adi_direct = adi_scheme_direct_source(dt, u0, num_steps, μ, ∇μX, ∇μY, ∇2μX, ∇2μY, source_direct)
        ...
        println("  ADI split error = $error_adi_split")
        println("  ADI direct error = $error_adi_direct")
        println("  FD error = $error_fd")
        println("  Mass conservation check:")
        println("    ADI split: $(sum(u_adi_split))")
        println("    ADI direct: $(sum(u_adi_direct))")
    end
    
    # Plot convergence
    p1 = plot(dt_values, [errors_lie errors_strang errors_adi_split errors_adi_direct errors_fd], 
              xlabel="Time step", ylabel="L2 Error", 
              label=["Lie" "Strang" "ADI Split" "ADI Direct" "FD"], 
              xscale=:log10, yscale=:log10,
              marker=:circle, title="MMS Convergence - Potential 1 (Direct vs Split)")
    
    return p1, errors_lie, errors_strang, errors_adi_split, errors_adi_direct, errors_fd
end
...
function run_mms_verification_2_direct()
    ...
    p1 = plot(dt_values, [errors_lie errors_strang errors_adi_split errors_adi_direct errors_fd], 
              ...
              label=["Lie" "Strang" "ADI Split" "ADI Direct" "FD"], 
              xscale=:log10, yscale=:log10,
              marker=:circle, title="MMS Convergence - Potential 2 (Direct vs Split)")
    
    return p1, errors_lie, errors_strang, errors_adi_split, errors_adi_direct, errors_fd
end

# Example usage function
function compare_adi_approaches()
    println("Comparing ADI with direct source integration vs operator splitting...")
    
    # Run both verification tests
    p1, errors_lie_1, errors_strang_1, errors_adi_split_1, errors_adi_direct_1, errors_fd_1 = run_mms_verification_1_direct()
    p2, errors_lie_2, errors_strang_2, errors_adi_split_2, errors_adi_direct_2, errors_fd_2 = run_mms_verification_2_direct()
    
    # Display both plots
    display(p1)
    display(p2)
    
    return (p1, p2), (errors_adi_split_1, errors_adi_direct_1), (errors_adi_split_2, errors_adi_direct_2)
end
