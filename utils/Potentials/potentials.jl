...
function generate_potential_3(folder = "")
    # Generate potential field and derivatives
    μ = readdlm(folder * "pmf.in")

    # X-direction derivatives
    ∇μX = readdlm(folder * "damjux.in")
    ∇2μX = readdlm(folder * "d2amjux.in")

    # Y-direction derivatives
    ∇μY = readdlm(folder * "damjuy.in")
    ∇2μY = readdlm(folder * "d2amjuy.in")
