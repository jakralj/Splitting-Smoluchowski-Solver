...
function random_placement(number, hexagon_points, width, big_radius, center_x, center_y)
    ...
    for random_point in random_points
        ...
        x = random_point[1] + width / sqrt(3) * cos(deg2rad(alpha))
        y = random_point[2] + width / sqrt(3) * sin(deg2rad(alpha))
        push!(placed_circles, (Int(round(x)), Int(round(y))))
    end
    
    points_to_remove = []
    for point in placed_circles
        dist = distance(center_x, center_y, point[1], point[2])
        if dist >= big_radius
...
function place_neuron!(grid, center_x, center_y, inner_radius, outer_radius, type1, type2)
    ...
    y_start = max(1, Int(floor(center_y - outer_radius)))
    y_end = min(height, Int(ceil(center_y + outer_radius)))
    x_start = max(1, Int(floor(center_x - outer_radius)))
    x_end = min(width, Int(ceil(center_x + outer_radius)))
...
function place_capillary!(grid, center_x, center_y, radius, type1)
    ...
    y_start = max(1, Int(floor(center_y - radius)))
    y_end = min(height, Int(ceil(center_y + radius)))
    x_start = max(1, Int(floor(center_x - radius)))
    x_end = min(width, Int(ceil(center_x + radius)))
...
function hexagonal_place(center_x, center_y, radius, big_radius, ratio)
    ...
    function hexagon(x, y, width)
        points = [(x + width * cos(deg2rad(alpha)), y + width * sin(deg2rad(alpha))) 
                 for alpha in 0:60:300]
        push!(points, (x, y))
        return points
    end
    ...
    for row in -Int(big_radius/row_height):Int(big_radius/row_height)
        for col in -Int(big_radius/col_width):Int(big_radius/col_width)
...
function calculate_partial_derivatives(grid)
    ...
    grad_xx[height, width] = grid[height, width] - 2*grid[height, width-1] + grid[height, width-2]
    grad_yy[height, width] = grid[height, width] - 2*grid[height-1, width] + grid[height-2, width]
...
hexagon_width, neuron_points = hexagonal_place(center_x, center_y, outer_radius, 
                                               big_radius - outer_radius, target_ratio)
println("Number of neuron points: $(length(neuron_points))")

println("Trying to place capillaries...")
capillary_points = random_placement(num_capillary, neuron_points, hexagon_width, 
                                   big_radius, center_x, center_y)
...
writedlm("pmf.out", grid_pmf, ' ')
writedlm("sink.out", grid_sink, ' ')
writedlm("u.out", grid_u, ' ')
