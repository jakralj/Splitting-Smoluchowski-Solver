...
function _build_system!(
    ...
    rhs[i, j] = -(u_p*(-∇μ*α + β) + u_i*(∇2μ*γ - ϵ + inv_dt) + u_n*(∇μ*α + β))
    if i == 1
...
function evolve_x_gpu!(u, work, μ_gpu, ∇μx_gpu, ∇2μx_gpu, D, dx, dt, kbT)
    T = eltype(u)
    nx, ny = size(u)
    α = D/(4*dx*kbT)
    β = D/(2*dx^2)
...
function evolve_y_gpu!(u, work, μ_gpu, ∇μy_gpu, ∇2μy_gpu, D, dx, dt, kbT)
    T = eltype(u)
    nx, ny = size(u)
