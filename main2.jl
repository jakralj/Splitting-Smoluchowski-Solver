...
∇μX = [cos(x*dx) for y in 1:nx, x in 1:nx]
∇2μX = [-sin(x*dx) for y in 1:nx, x in 1:nx]

# Y-direction derivatives
∇μY = [-sin(y*dx) for y in 1:nx, x in 1:nx]
∇2μY = [-cos(y*dx) for y in 1:nx, x in 1:nx]
...
function run_example()
    ...
    compare_splitting_methods(u0, 100)
end
